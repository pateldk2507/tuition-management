  ...
  if (type !== "delete") {
    switch (table) {
      ...
      case "exam":
        ...
        break;
      case "parent":
        // no extra related data for parent
        relatedData = {};
        break;
      case "lesson":
        ...
        break;
      case "assignment":
        const assignmentLessons = await prisma.lesson.findMany({ select: { id: true, name: true } });
        relatedData = { lessons: assignmentLessons };
        break;
      case "result":
        const resultStudents = await prisma.student.findMany({ select: { id: true, name: true, surname: true } });
        const resultExams = await prisma.exam.findMany({ select: { id: true, title: true } });
        const resultAssignments = await prisma.assignment.findMany({ select: { id: true, title: true } });
        relatedData = { students: resultStudents, exams: resultExams, assignments: resultAssignments };
        break;
      case "attendance":
        const attendanceStudents = await prisma.student.findMany({ select: { id: true, name: true, surname: true } });
        const attendanceLessons = await prisma.lesson.findMany({ select: { id: true, name: true } });
